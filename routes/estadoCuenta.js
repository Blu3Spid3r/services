...
// GET /services/estado-cuenta
router.get('/', async (req, res) => {
  const sub = req.auth?.sub
  console.log('🔐 [EstadoCuenta] Token sub:', sub)
  ...
  try {
    ...
    // Ejecutar el stored procedure (ejemplo: dbo.EstadoCuentaConsultaProc)
    const movimientos = await pool.request()
      .input('UserToken', sql.UniqueIdentifier, userToken)
      .execute('dbo.EstadoCuentaConsultaProc')

    res.json(movimientos.recordset)
  } catch (err) {
    console.error('❌ Error en Estado de Cuenta:', err)
    res.status(500).json({ error: 'Error interno al consultar estado de cuenta' })
  }
})
