...
router.get('/', async (req, res) => {
  ...
  console.log('🔐 [Facturas Por Vencer] Token sub:', sub)
  ...
  try {
    ...
    const facturas = await pool.request()
      ...
      .execute('dbo.FacturasPorVencerConsultaProc')

    res.json(facturas.recordset)
    console.log(facturas.recordset)
  } catch (err) {
    console.error('❌ Error en Facturas Por Vencer:', err)
    res.status(500).json({ error: 'Error interno al consultar facturas por vencer' })
  }
})
