...
// GET /services/productos
router.get('/', async (req, res) => {
  const sub = req.auth?.sub
  console.log('🔐 [Productos] Token sub:', sub)
  ...
  try {
    ...
    // Ejecutar el stored procedure (ejemplo: dbo.ProductoConsultaProc)
    const productos = await pool.request()
      .input('UserToken', sql.UniqueIdentifier, userToken)
      .execute('dbo.ProductoConsultaProc')
