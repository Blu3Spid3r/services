...
// GET /services/deposito-tipo
router.get('/', async (req, res) => {
  const sub = req.auth?.sub
  console.log('🔐 [Deposito Tipos] Token sub:', sub)
  ...
  try {
    ...
    // Ejecutar el stored procedure (ejemplo: dbo.DepositoTipoConsultaProc)
    const facturaTipos = await pool.request()
      .input('UserToken', sql.UniqueIdentifier, userToken)
      .execute('dbo.DepositoTipoConsultaProc')

    res.json(facturaTipos.recordset)
  } catch (err) {
    console.error('❌ Error en Deposito Tipos:', err)
    res.status(500).json({ error: 'Error interno al consultar deposito tipos' })
  }
})
